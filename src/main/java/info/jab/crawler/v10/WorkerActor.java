...
/**
 * Worker actor that processes individual URL crawl requests using virtual threads.
 *
 ...
 *
 * The actor processes URLs using virtual threads for lightweight concurrency
...
public class WorkerActor {
     ...
     * @param executor virtual thread executor for async processing
    ...
    /**
     * Processes a URL crawl request using virtual threads.
     *
    ...
    private Document fetchAndParsePage(String url) throws IOException {
        return Jsoup.connect(url)
            .timeout(timeoutMs)
            .userAgent("Mozilla/5.0 (Educational Virtual Thread Actor Crawler)")
